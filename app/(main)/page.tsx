...
import { 
  ...
  Briefcase, Megaphone, UserCheck, LayoutGrid
} from 'lucide-react';

// --- DATA IMPORTS ---
import { getActiveProduction, getShowById, getAssignments, getCreativeTeam } from '@/app/lib/baserow';
import { MOCK_CLASSES } from '../lib/educationFillerData';
import CreativeTeam from '@/app/components/dashboard/CreativeTeam';

// --- CONSTANTS: SEASON STAFF ---
// These are static "Pillars" of the season, separate from the show-specific team.
const SEASON_STAFF = [
...
export default async function DashboardPage() {
  // 1. DETERMINE CONTEXT (Cookies)
  const cookieStore = await cookies();
  const activeId = Number(cookieStore.get('active_production_id')?.value);

  // 2. FETCH DATA (Parallelized for Speed)
  // We need to resolve the Show ID first to know which Creative Team to fetch
  const show = activeId ? await getShowById(activeId) : await getActiveProduction();
  const activeProdId = show?.id || 0;

  const [assignments, creativeTeam] = await Promise.all([
      getAssignments(activeProdId),
      getCreativeTeam(activeProdId) // <--- Dynamic Fetch from Baserow
  ]);
  
  // --- 3. CALCULATE STATS ---
  
  // Cast Count (Unique Heads)
  ...
  // Academy Count (Total Enrollment)
  const academyCount = MOCK_CLASSES.reduce((acc, c) => acc + c.enrolled, 0);
  
  // Ministry Family Units (Estimated: 1.3 students per family)
  ...
  const getShowTheme = (title: string) => {
    ...
    if (t.includes('frozen') || t.includes('elf') || t.includes('christmas')) return { icon: <Snowflake size={220} />, color: 'text-sky-300', bg: 'from-sky-900/40 to-blue-900/30', accent: 'text-sky-200' };
    if (t.includes('beauty') || t.includes('cinderella') || t.includes('princess')) return { icon: <Crown size={220} />, color: 'text-purple-400', bg: 'from-purple-900/40 to-fuchsia-900/20', accent: 'text-amber-400' };
    if (t.includes('wonka') || t.includes('matilda') || t.includes('seuss')) return { icon: <Sparkles size={220} />, color: 'text-pink-500', bg: 'from-pink-900/30 to-rose-900/20', accent: 'text-pink-400' };
    if (t.includes('newsies') || t.includes('annie') || t.includes('oliver')) return { icon: <Newspaper size={220} />, color: 'text-zinc-400', bg: 'from-zinc-800/40 to-zinc-900', accent: 'text-zinc-200' };
    if (t.includes('oklahoma') || t.includes('music man') || t.includes('fiddler')) return { icon: <Sun size={220} />, color: 'text-amber-600', bg: 'from-amber-900/30 to-orange-900/20', accent: 'text-amber-500' };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 p-6 pb-20 overflow-y-auto custom-scrollbar space-y-8">
      ...
      {/* ========================================= */}
      {/* 1. ACTIVE PRODUCTION HERO (Dynamic)       */}
      {/* ========================================= */}
      <div className={`relative overflow-hidden bg-zinc-900 border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl transition-all duration-1000 bg-gradient-to-br ${theme.bg}`}>
        ...
        <div className="relative z-10 max-w-5xl">
            ...
            {/* Dynamic Stats & Creative Team Row */}
            <div className="flex flex-col xl:flex-row xl:items-center gap-6 xl:gap-8">
                ...
                {/* THE DYNAMIC CREATIVE TEAM WIDGET */}
      ...
      <div className="bg-zinc-900/50 border border-white/5 rounded-3xl p-6 md:p-8 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 relative z-10">
              ...
              {/* Core Season Staff (Static Pillars) */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10">
              {/* Academy Card */}
              <div className="bg-black/40 rounded-2xl p-5 border border-white/5 flex flex-col justify-between h-32 hover:bg-black/60 transition-colors">
                  <div className="p-2 w-fit rounded-lg bg-blue-500/10 text-blue-400 mb-2"><GraduationCap size={20} /></div>
                  <div>
              ...
              {/* Cast Card */}
              <div className="bg-black/40 rounded-2xl p-5 border border-white/5 flex flex-col justify-between h-32 hover:bg-black/60 transition-colors">
                  <div className="p-2 w-fit rounded-lg bg-purple-500/10 text-purple-400 mb-2"><Stars size={20} /></div>
                  <div>
                      <div className="text-3xl font-black text-white">{castCount}</div>
              ...
              {/* Families Card */}
              <div className="bg-black/40 rounded-2xl p-5 border border-white/5 flex flex-col justify-between h-32 hover:bg-black/60 transition-colors">
                  <div className="p-2 w-fit rounded-lg bg-emerald-500/10 text-emerald-400 mb-2"><Home size={20} /></div>
                  <div>
              ...
              {/* Ministry Status */}
              <div className="bg-black/40 rounded-2xl p-5 border border-white/5 flex flex-col justify-between h-32 hover:bg-black/60 transition-colors">
                  <div className="p-2 w-fit rounded-lg bg-amber-500/10 text-amber-400 mb-2"><UserCheck size={20} /></div>
                  <div>
                      <div className="text-3xl font-black text-white">4 / 4</div>
      ...
      <div className="mt-12 pt-10 border-t border-white/5 flex flex-col items-center gap-3">
          ...
          <div className="flex gap-2">
            <div className="px-3 py-1 bg-zinc-900 border border-white/5 rounded-full text-[9px] text-zinc-500 font-black uppercase tracking-widest">Build 1.3.0</div>

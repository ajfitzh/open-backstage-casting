...
export default async function CastListPage({ params }: { params: { tenant: string, id: string } }) {
  ...
  // 🟢 SAFE FETCH: Catch any errors and default to an empty array
  const [show, assignmentsData] = await Promise.all([
    getShowById(tenant, productionId),
    getAssignments(tenant, productionId).catch(() => [])
  ]);

  // 🟢 SAFE ARRAY CHECK: If Baserow returned an error object, force it to be an array
    ...
    <div className="min-h-screen bg-zinc-950 text-white p-8 pb-20">
      <div className="max-w-5xl mx-auto mb-10">
        ...
        <h1 className="text-4xl font-black uppercase tracking-tighter text-white mb-2">
          {show?.title || "Production"} <span className="text-zinc-600">Cast</span>
        </h1>
      ...
      {castList.length === 0 ? (
         <div className="max-w-5xl mx-auto text-center py-20 border-2 border-dashed border-zinc-800 rounded-3xl">
            <h3 className="text-xl font-bold text-zinc-600 uppercase tracking-widest">No Cast Assigned Yet</h3>
            <p className="text-zinc-500 mt-2">The cast list will appear here once casting is finalized.</p>
         </div>
      ) : (

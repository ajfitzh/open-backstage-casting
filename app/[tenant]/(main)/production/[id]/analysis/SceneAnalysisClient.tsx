...
export default function SceneAnalysisClient({ scenes }: { scenes: Scene[] }) {
  ...
  const handleUpdate = (id: number, type: 'music' | 'dance' | 'block', value: number) => {
    setLocalScenes(prev => prev.map(s => {
        if (s.id !== id) return s;
        return { ...s, load: { ...s.load, [type]: value } };
    }));
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pb-32">
      ...
      {/* Sticky Save Bar */}
      <div className={`fixed bottom-8 right-8 z-50 transition-all duration-300 ${dirtyIds.size > 0 ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}`}>
         <button 
           onClick={saveChanges}
           disabled={saving}
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        <Link href="/" className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors text-sm font-bold mb-4 w-fit">
        ...
        <div className="bg-zinc-900 border border-white/10 rounded-xl p-6 flex gap-4">
           ...
           <div>
              ...
              <p className="text-zinc-400 text-sm mt-1">
                ...
                This data powers the "Show Readiness" burn-up chart on your dashboard.
              </p>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
           {localScenes.map((scene) => (
             <div key={scene.id} className={`bg-zinc-900/50 border p-6 rounded-xl transition-all ${dirtyIds.has(scene.id) ? 'border-blue-500/50 bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.15)]' : 'border-white/5 hover:border-white/10'}`}>

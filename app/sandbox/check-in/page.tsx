...
const generateMassiveRoster = () => {
  ...
  for (let i = 0; i < 76; i++) {
    ...
    roster.push({
      ...
      family: { parents: [`${firstNames[(i + 5) % firstNames.length]} ${lName}`], siblings: [] },
      ...
      auditionPrep: { 
        monologue: i % 2 === 0 ? 'Comedic 1' : 'Dramatic 2', 
        songTitle: i % 3 === 0 ? 'Tomorrow' : 'Stars', 
        musicProvided: Math.random() > 0.3 
      },
      lobbyNote: ''
    });
  }
...
export default function SandboxCheckIn() {
  ...
  // Hydration Fix: Only generate the random roster after mounting
  useEffect(() => {
  ...
  const handleReassign = (id: string, field: 'timeSlot' | 'auditionDay', value: string) => {
  ...
  // Avoid rendering content that depends on random data until mounted
  if (!hasMounted) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-4 md:p-8 font-sans relative text-slate-200">
      ...
      <div className="max-w-4xl mx-auto flex items-center justify-between mb-6">
        ...
        <span className="px-3 py-1 bg-indigo-500/10 text-indigo-400 text-[10px] font-black uppercase tracking-widest rounded-full border border-indigo-500/20">Sandbox Desk</span>
      </div>
      ...
      <div className="max-w-4xl mx-auto bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 md:p-6 border-b border-slate-800 bg-slate-900 shrink-0 z-20 space-y-4">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <div className="relative w-full md:max-w-xs shrink-0">
              ...
              <input type="text" placeholder="Search 76 students..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-slate-950 border border-slate-800 text-white pl-10 pr-4 py-3 rounded-lg focus:ring-1 ring-indigo-500 outline-none w-full text-sm"/>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar items-center">
              ...
              {ALL_TIME_SLOTS.map(slot => {
                const count = getSlotCount(slot, activeDayFilter);
                return (
                  <button key={slot} onClick={() => setActiveTimeFilter(slot)} className={`px-3 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all border whitespace-nowrap ${activeTimeFilter === slot ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg' : 'bg-slate-950 text-slate-500 border-slate-800 hover:text-slate-200'}`}>
                    {slot} <span className="opacity-40 ml-1">({count})</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="p-2 md:p-4 overflow-y-auto flex-1 custom-scrollbar">
          {Object.keys(groupedStudents).length === 0 ? (
            <div className="text-center py-20 opacity-30 italic text-sm">No students scheduled for {activeDayFilter} {activeTimeFilter !== 'All' ? `at ${activeTimeFilter}` : ''}</div>
          ) : (
                      ...
                      <div className="flex items-center gap-4 min-w-0 pr-4">
                        <div className="shrink-0 relative">
                          <img 
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setMaximizedImage(student.avatar);
                            }}
                            className="w-12 h-12 md:w-14 md:h-14 rounded-full border-2 border-slate-700 bg-slate-800 object-cover shadow-md cursor-zoom-in hover:border-indigo-400 transition-colors" 
                          />
      ...
      {activeStudent && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-2 md:p-4 z-40" onClick={() => setActiveStudent(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[95vh] md:max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 md:p-6 border-b border-slate-800 flex justify-between items-start shrink-0 bg-slate-900/50">
              <div className="flex gap-4 items-center">
                <img 
                  ...
                  className="w-16 h-16 rounded-full border-2 border-slate-700 shadow-lg hidden md:block cursor-zoom-in hover:border-indigo-400 transition-colors" 
                />
            ...
            <div className={`p-4 md:p-6 space-y-6 overflow-y-auto custom-scrollbar ${activeStudent.status !== 'Pending' ? 'opacity-40 pointer-events-none' : ''}`}>
              ...
              <div className="space-y-3">
                ...
                {activeStudent.missingForms.length === 0 ? (
                  <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-400 text-xs font-bold text-center">All Paperwork Complete</div>
                ) : (
      ...
      {maximizedImage && (
        <div 
          className="fixed inset-0 z-[110] bg-slate-950/90 backdrop-blur-xl flex items-center justify-center p-4 cursor-zoom-out"
          onClick={() => setMaximizedImage(null)}
        >
          <div className="relative group">
            <img 
              src={maximizedImage} 
              alt="Maximized Headshot" 
              className="max-w-full max-h-[85vh] rounded-2xl border-4 border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-200"
            />
            <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-white/10 px-4 py-1 rounded-full text-white/50 text-[10px] font-black uppercase tracking-[0.2em] opacity-0 group-hover:opacity-100 transition-opacity">

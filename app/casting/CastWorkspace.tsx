    ...
    <main className="bg-zinc-950 flex flex-col overflow-hidden relative h-full">
      <header className="p-4 border-b border-white/5 bg-zinc-900/80 backdrop-blur-md z-10 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-4">
            <h1 className="text-xl font-black italic uppercase">Cast Grid</h1>
            
            {/* LEGEND */}
            <div className="flex items-center gap-3 px-3 py-1 bg-zinc-900 rounded-full border border-white/5">
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-500"></div><span className="text-[9px] font-bold text-zinc-400 uppercase">Scene</span></div>
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-blue-500"></div><span className="text-[9px] font-bold text-zinc-400 uppercase">Song</span></div>
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-purple-500"></div><span className="text-[9px] font-bold text-zinc-400 uppercase">Dance</span></div>
            </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="space-y-4 pb-20">
            ...
            {/* GRID HEADER */}
            <div className="grid grid-cols-[280px_1fr_30px] gap-4 mb-2 sticky top-0 z-20 pr-2">
                <div></div> 
            ...
            {/* ROLES ROWS */}
            {roles.map((role) => (
                <div key={role.id} className="group grid grid-cols-[280px_1fr_30px] gap-4 p-2 bg-zinc-900/30 border border-white/5 rounded-xl hover:bg-zinc-900 transition-all items-start">
                    ...
                    >
                         <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-black uppercase text-zinc-400 truncate max-w-[180px]" title={safeString(role.name)}>{safeString(role.name)}</span>
                            <span className="text-[9px] font-bold px-1.5 rounded bg-black/40 text-zinc-500 shrink-0">{role.sceneIds.length} Scn</span>
                         </div>

                        {/* ACTOR CHIPS */}
                        <div className="flex flex-wrap gap-1.5">
                            {role.actors.length === 0 && <p className="text-[10px] text-zinc-600 italic py-1 w-full text-center">Drag actors here...</p>}
                            ...
                            {role.actors.map((actor: any) => {
                                ...
                                return (
                                    ...
                                    >
                                        <img src={actor.Headshot?.[0]?.url || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"} className="w-4 h-4 rounded-full object-cover shrink-0" />
                                        <span className="text-[9px] font-bold truncate">{safeString(actor.Performer)}</span>
                                        <button 
                    ...
                    {/* RIGHT: ACTIONS */}
                    <div className="flex flex-col gap-1 items-center opacity-0 group-hover:opacity-100 transition-opacity pt-1">

...
export default function ChemistryWorkspace({ roles, onDropActor, onRemoveActor, onConfirmRole }: Props) {
  ...
  const getScore = (actor: any) => {
      const val = getValue(actor.Score, null);
      return val || (Math.floor(Math.random() * (10 - 7) + 7) + Math.round(Math.random() * 10) / 10);
  };

  const getHeight = (actor: any) => {
      const val = getValue(actor.Height, null);
      return val || `${Math.floor(Math.random() * (6 - 4) + 4)}'${Math.floor(Math.random() * 11)}"`;
  };

  const getAge = (actor: any) => getValue(actor.Age, Math.floor(Math.random() * (18 - 13) + 13));
  
  const getExperience = (actor: any) => getValue(actor.Experience, Math.floor(Math.random() * 15));

  const getConflicts = (actor: any) => {
      const val = actor.Conflicts;
      if (!val) return [];
      let list: any[] = [];
      if (Array.isArray(val)) list = val;
      else if (typeof val === 'string') list = val.split(',');
      else list = [val];
      return list.map(item => String(getValue(item, ""))).filter(s => s !== "");
  };
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-950 relative overflow-hidden">
      ...
      {/* HEADER */}
      <header className="p-4 border-b border-white/5 bg-zinc-900/50 flex justify-between items-center shrink-0">
      ...
      {/* COMPARISON STAGE */}
      <div className="flex-1 overflow-x-auto overflow-y-auto p-8 custom-scrollbar bg-zinc-950 space-y-12">
            
            {visibleRoles.length === 0 && (
                <div className="w-full h-full flex flex-col items-center justify-center text-zinc-600 opacity-50">
            ...
            {visibleRoles.map(role => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3 mb-4 pl-2">
                         <h2 className="text-2xl font-black uppercase text-white tracking-tighter italic">{safeString(role.name)}</h2>
                        ...
                        {role.actors.length === 0 ? (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr>
                                            <th className="p-4 w-32 bg-zinc-900/50 border-b border-r border-white/5 text-[10px] font-black uppercase text-zinc-500 tracking-wider text-right align-bottom">Candidate</th>
                                            {role.actors.map((actor: any) => {
                                                const isSelected = role.selectedActorId === actor.id;
                                                return (
                                                <th key={actor.id} className={`p-4 border-b border-r border-white/5 min-w-[160px] w-[160px] relative group/col ${isSelected ? 'bg-purple-900/10' : ''}`}>
                                                     <div className={`relative aspect-[3/4] rounded-lg overflow-hidden border shadow-lg mb-2 transition-all ${isSelected ? 'border-purple-500 ring-2 ring-purple-500/50' : 'border-white/10'}`}>
                                                        <img src={actor.Headshot?.[0]?.url || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png"} className="w-full h-full object-cover" />
                                                        
                                                        {/* REMOVE */}
                                                        <button onClick={() => onRemoveActor(role.id, actor.id)} className="absolute top-1 right-1 p-1 bg-black/60 text-zinc-400 hover:text-red-400 rounded-full opacity-0 group-hover/col:opacity-100 transition-opacity"><X size={12} /></button>
                                            ...
                                            <th className="p-4 border-b border-white/5 min-w-[100px] w-[100px] bg-zinc-900/30 border-dashed border-l border-white/10"></th>
                                        </tr>
                                    </thead>
                                    <tbody className="text-xs font-bold text-zinc-300">
                                        ...
                                        {/* METRIC ROWS */}
                                        {[
                                            { label: 'Callback Score', val: getScore, format: (v: any) => <span className={`px-2 py-1 rounded ${v >= 9 ? 'bg-emerald-500/20 text-emerald-400' : v >= 8 ? 'bg-blue-500/20 text-blue-400' : 'text-zinc-400'}`}>{v}</span> },
                                            ...
                                            { label: 'Experience', val: getExperience, format: (v: any) => <div className="flex items-center justify-center gap-1 text-zinc-400"><Trophy size={10} className="text-amber-500" /> {v} Shows</div> },
                                        ].map((row, i) => (
                                            <tr key={i} className="hover:bg-zinc-800/30 transition-colors">
                                                <td className="p-3 border-b border-r border-white/5 text-right text-zinc-500 uppercase text-[10px]">{row.label}</td>
                                                {role.actors.map((actor: any) => (
                                        ...
                                        <tr className="bg-red-900/5">
                                            <td className="p-3 border-r border-white/5 text-right text-red-500/70 font-black uppercase text-[10px] align-top pt-4">Conflicts</td>
                                            {role.actors.map((actor: any) => {

...
interface ActorProfileModalProps {
  actor: any;
  grades: any; 
  timeline?: React.ReactNode; // <--- NEW SLOT FOR THE SCENE CHART
  onClose: () => void;
}

export default function ActorProfileModal({ actor, grades, timeline, onClose }: ActorProfileModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-sm flex items-center justify-center p-0 md:p-4">
      <div className="bg-zinc-950 w-full h-full md:max-w-4xl md:h-[800px] md:rounded-3xl md:border md:border-white/10 overflow-hidden shadow-2xl flex flex-col md:flex-row relative">
        ...
        <div className="p-6 md:p-8 border-b md:border-b-0 md:border-r border-white/5 bg-zinc-900/50 w-full md:w-[35%] flex flex-row md:flex-col items-center md:justify-start gap-6 shrink-0">
          ...
          <div className="flex-1 md:w-full md:text-center">
              <h2 className="text-xl md:text-2xl font-black italic uppercase tracking-tight leading-tight text-white">{actor.name}</h2>
          ...
          {/* Radar Chart */}
          <div className="hidden md:block w-full h-48 mt-auto relative opacity-80 hover:opacity-100 transition-opacity">
             <p className="absolute -top-2 w-full text-[9px] text-center text-zinc-500 font-black uppercase tracking-[0.2em]">Audition Stats</p>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar pb-24 md:pb-8">
            {activeTab === "insights" ? (
              <div className="space-y-6 md:space-y-8">
                ...
                {/* 0. THE MERGED TIMELINE (Injected here) */}
                {timeline && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3 bg-zinc-900 rounded-xl border border-white/5">
                    ...
                    <p className="text-xs font-bold text-zinc-300 line-clamp-1">{actor.song || "N/A"}</p>
                  </div>
                  <div className="p-3 bg-zinc-900 rounded-xl border border-white/5">
                    <p className="text-[9px] font-black text-emerald-400 uppercase mb-1 tracking-tighter">Monologue</p>
                    <p className="text-xs font-bold text-zinc-300 line-clamp-1">{actor.monologue || "N/A"}</p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="bg-zinc-900/40 p-4 rounded-xl border border-white/5">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-[10px] font-bold text-blue-400 uppercase">Director</p>
                        </div>
                        <p className="text-xs text-zinc-300 italic">"{grades?.actingNotes || "No notes."}"</p>
                    </div>
                    <div className="bg-zinc-900/40 p-4 rounded-xl border border-white/5">
                        <div className="flex items-center gap-2 mb-2">
                            <Mic2 size={12} className="text-purple-400" />
                            <p className="text-[10px] font-bold text-purple-400 uppercase">Music</p>
                        </div>
                        <p className="text-xs text-zinc-300 italic">"{grades?.vocalNotes || "No notes."}"</p>
                    </div>
                    <div className="bg-zinc-900/40 p-4 rounded-xl border border-white/5">
                        <div className="flex items-center gap-2 mb-2">
                            <Move size={12} className="text-emerald-400" />
                            <p className="text-[10px] font-bold text-emerald-400 uppercase">Dance</p>
                        </div>
                        <p className="text-xs text-zinc-300 italic">"{grades?.choreoNotes || "No notes."}"</p>
                    </div>
                    <div className="bg-zinc-900/40 p-4 rounded-xl border border-white/5">
                        <div className="flex items-center gap-2 mb-2">
                            <FileText size={12} className="text-amber-400" />
                            <p className="text-[10px] font-bold text-amber-400 uppercase">Admin</p>
                        </div>
                        <p className="text-xs text-zinc-300 italic">"{grades?.adminNotes || "No notes."}"</p>
                    </div>
                  </div>
                </section>
...
function ConflictAnalyzer({ conflicts }: { conflicts: any }) {
  const data = useMemo(() => {
    ...
    const items = rawList.map((str, i) => {
        ...
        const isSevere = lower.includes("absent") || lower.includes("missing") || lower.includes("vacation");
        const isMinor = lower.includes("late") || lower.includes("early");
    ...
    <div className="space-y-4">
        <div className="flex gap-3">
            <div className={`flex-1 p-3 rounded-xl border flex items-center gap-3 ${data.severe > 0 ? 'bg-red-500/10 border-red-500/30' : 'bg-zinc-900 border-white/5'}`}>
                <div className={`p-2 rounded-lg ${data.severe > 0 ? 'bg-red-500 text-white' : 'bg-zinc-800 text-zinc-500'}`}><AlertOctagon size={16} /></div>
                <div>
            ...
            <div className={`flex-1 p-3 rounded-xl border flex items-center gap-3 ${data.minor > 0 ? 'bg-amber-500/10 border-amber-500/30' : 'bg-zinc-900 border-white/5'}`}>
                <div className={`p-2 rounded-lg ${data.minor > 0 ? 'bg-amber-500 text-black' : 'bg-zinc-800 text-zinc-500'}`}><Clock size={16} /></div>
                <div>

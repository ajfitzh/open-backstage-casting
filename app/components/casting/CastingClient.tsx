...
import { 
  ...
  Archive, Undo, Circle, AlertCircle, CheckCircle2
} from "lucide-react";
...
function getStudentCompliance(studentId: number, rows: AssignmentRow[], allScenes: Scene[]) {
  ...
  // 2. Gather unique Scene IDs
  const mySceneIds = new Set<number>();
  myAssignments.forEach(row => {
    // Check pending first (draft), then saved
...
// ------------------------------------------------------------------
// 🧩 SUB-COMPONENT: ROSTER SIDEBAR (The Funnel)
// ------------------------------------------------------------------
function RosterSidebar({ 
  ...
  // 1. Calculate Compliance for ALL students (Memoized for performance)
  ...
  const filtered = students.filter(s => {
    ...
    if (showReleased) {
        return isReleased; // Only show released
    }
    if (isReleased) return false; // Hide released usually
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar bg-zinc-950/50">
        ...
        {filtered.map(student => {
           ...
           return (
            <div 
                key={student.id}
                draggable={!showReleased} // Disable drag if released
                onClick={() => onSelect(student)}
                onDragStart={(e) => e.dataTransfer.setData("actorId", String(student.id))}
                className={`group bg-zinc-900 border-l-4 p-3 rounded-r-lg flex items-center gap-3 cursor-pointer hover:bg-zinc-800 transition-all shadow-sm ${borderClass} ${showReleased ? 'opacity-60 grayscale' : ''}`}
            >
                {/* AVATAR */}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                        ...
                        {/* THE RELEASE BUTTON (Visible on Hover) */}
                        <button 
                    ...
                    {/* COMPLIANCE BADGES (Only show if assigned or at-risk) */}
...
export default function CastingClient({ 
  ...
  // ✅ Toggle Release Handler
  const handleToggleRelease = (studentId: number) => {
    setReleasedIds(prev => {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] relative">
      ...
      {/* 1. ROSTER (Updated Prop Signature) */}
      <RosterSidebar 
        students={roster} 
        rows={rows}
        allScenes={allScenes}
...
// Re-paste ChemistryWorkspace code or import it if separate (omitted for brevity as it was unchanged from previous turn aside from props)
const METRICS = [

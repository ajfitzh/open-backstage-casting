...
import { CheckCircle2, Circle, User, DollarSign, FileText, Camera, Ruler, AlertCircle } from 'lucide-react';
    ...
    return (
      <div className="bg-zinc-950 text-zinc-50 p-6 min-h-screen flex flex-col items-center justify-center">
        ...
        <div className="p-12 text-center text-zinc-500 border border-zinc-800 rounded-lg bg-zinc-900/50 max-w-md">
          ...
          <p className="text-sm mt-2">We couldn't find any cast members for <strong className="text-zinc-300">{productionTitle}</strong>.</p>
        </div>
      </div>
    );
  }
...
return (
    <div className="bg-zinc-950 text-zinc-50 p-6 pb-32">
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-800 bg-zinc-950/50">
            {students.sort((a, b) => a.name.localeCompare(b.name)).map((student, index) => (
              <tr key={`${student.id}-${index}`} className="hover:bg-zinc-900/80 transition-colors group">
                <td className="px-4 py-4 font-medium flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 group-hover:border-emerald-500/50 border border-zinc-700 transition-colors">
                    {student.avatar ? <img src={student.avatar} alt={student.name} className="w-full h-full object-cover"/> : <User size={16} />}
                  </div>
  ...
  <td className="px-4 py-4 text-center">
    <button 
      className={`inline-flex items-center justify-center p-2 rounded-full transition-all duration-300 ${
        checked ? 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20' : 'text-zinc-700 hover:text-zinc-500 hover:bg-zinc-800'
      }`}

...
import { CheckCircle2, Circle, ArrowRight, Mic2, Users, SlidersHorizontal, Calendar } from 'lucide-react';
    ...
    <div className="w-full bg-zinc-900 border border-white/10 rounded-xl p-6 mb-8">
      <div className="flex justify-between items-end mb-6">
        <div>
            ...
            <p className="text-zinc-400 text-sm mt-1">Current Phase: <span className="text-blue-400 font-bold">{steps.find(s => !s.isComplete)?.label || "Production Active"}</span></p>
        </div>
         ...
         {steps.map((step, i) => {
            ...
            return (
                <Link 
                    key={step.id} 
                    href={step.link}
                    className={`group relative flex flex-col items-center text-center p-4 rounded-xl transition-all border ${
                        isNext 
                ...
                >
                    ...
                    <h3 className={`text-sm font-bold ${step.isComplete ? 'text-zinc-400' : 'text-white'}`}>{step.label}</h3>
                    <p className="text-xs text-zinc-500 mt-1">{step.desc}</p>

                    {isNext && (
                        <div className="absolute -bottom-3 bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm">

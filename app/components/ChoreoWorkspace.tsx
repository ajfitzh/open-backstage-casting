...
const TAGS = [
  ...
  { id: "#Warning", label: "Attitude", icon: AlertTriangle, color: "bg-red-600 border-red-400" },
];
...
export default function ChoreoWorkspace({ 
  ...
  // Local state for the "Lineup" order
  ...
  // Sync lineup if people change (e.g. searching)
  ...
  const moveLineup = (fromIndex: number, direction: 'left' | 'right') => {
    ...
    setLineup(newLineup);
    setSelectedIndex(toIndex); // Follow the student
  };
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      ...
      {/* 1. TOP: THE "STAGE" (Active Student) */}
      <div className="flex-1 relative flex flex-col p-4">
         ...
         <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-2 z-10 pointer-events-none">
            <button 
                ...
                className="pointer-events-auto p-4 bg-black/50 backdrop-blur rounded-full text-white hover:bg-white/20 disabled:opacity-0"
             ...
             <div className="relative w-40 h-40 md:w-56 md:h-56 rounded-full overflow-hidden border-4 border-white/10 shadow-2xl">
                 ...
                 {activeGrade.dance > 0 && (
                     <div className="absolute inset-0 bg-black/40 flex items-center justify-center backdrop-blur-[2px]">
                         <span className="text-6xl font-black text-white drop-shadow-lg">{activeGrade.dance}</span>
                     </div>
                 )}
             </div>
             
             <div className="text-center z-10">
                 <h2 className="text-3xl font-black uppercase italic tracking-tight">{activeStudent.name}</h2>
                 <p className="text-zinc-400 font-bold text-sm uppercase tracking-widest mt-1">#{activeStudent.id.toString().slice(-3)} • {activeStudent.age} • {activeStudent.height}</p>
             </div>
         </div>
      </div>

      {/* 2. MIDDLE: CONTROL PAD (Thumb Friendly) */}
      <div className="bg-zinc-900 border-t border-white/10 p-4 pb-8 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-20">
          
          {/* LEVEL SELECTOR (One Tap) */}
          <div className="grid grid-cols-4 gap-2 mb-6">
              {LEVELS.map((lvl) => (
                  <button
                      key={lvl.val}
                      onClick={() => handleScore(lvl.val)}
                      className={`h-16 rounded-xl flex flex-col items-center justify-center border-2 transition-all active:scale-95
                      ...
                      `}
                  >
                      <span className="text-xl font-black">{lvl.val}</span>
                      <span className="text-[9px] font-bold uppercase">{lvl.label}</span>
                  </button>
              ))}
          </div>
              ...
              {TAGS.map((tag) => {
                  ...
                  return (
                      <button
                          ...
                          className={`flex items-center gap-2 px-4 py-3 rounded-full border transition-all active:scale-95
                              ${isActive 
      ...
      {/* 3. BOTTOM: LINEUP SCROLL (The "Line") */}
      <div className="bg-black border-t border-white/10 h-24 overflow-x-auto custom-scrollbar shrink-0">
          <div className="flex items-center h-full px-4 gap-2">
              {lineup.map((person, i) => (
                  <div 
                    key={person.id} 
                    onClick={() => setSelectedIndex(i)}
                    className={`relative w-16 h-16 shrink-0 rounded-lg overflow-hidden cursor-pointer transition-all
                  ...
                  >
                      ...
                      {/* Reorder Buttons (Visible only on active item) */}
                      {selectedIndex === i && (
                          <>
